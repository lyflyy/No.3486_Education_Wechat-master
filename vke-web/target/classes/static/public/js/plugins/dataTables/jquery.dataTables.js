...
(/** @lends <global> */function( $ ) {
	...
	var _pluck = function ( a, prop, prop2 ) {
		...
		// Could have the config.test in the loop for slightly smaller code, but speed
		// is essential here
		if ( prop2 !== undefined ) {
			for ( ; i<ien ; i++ ) {
				if ( a[i] && a[i][ prop ] ) {
	...
	function _fnGetObjectDataFn( mSource )
	{
			      ...
			      mSource.indexOf('[') !== -1 || mSource.indexOf('(') !== -1) )
		{
			 ...
			 * level down. On each loop we config.test for undefined, and if found immediately
	...
	function _fnSort ( oSettings )
	{
		...
		if ( _fnDataSource( oSettings ) != 'ssp' && aSort.length !== 0 )
		{
			 ...
			 *  }
			 * Basically we have a config.test for each sorting column, if the data in that column is equal,
			 * config.test the next column. If all columns match, then we use a numeric sort on the row
	...
	DataTable = function( options )
	{
		 ...
		 *      oTable.fnFilter( 'config.test string' );
		 *    } );
		 */
	 ...
	 *      selector for the table to config.test. Note that if more than more than one
